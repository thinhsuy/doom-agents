...
## Working guidelines (behavioral)

Behavioral guidelines to reduce common LLM coding mistakes. These apply on top of the
project-specific rules below. **Tradeoff:** they bias toward caution over speed — for
...
When your changes create orphans: remove imports/variables/functions that YOUR changes
made unused, but don't remove pre-existing dead code unless asked. The test: every changed
...
For multi-step tasks, state a brief plan (`[step] → verify: [check]` per line). Strong
success criteria let you loop independently; weak criteria ("make it work") require constant
clarification.
